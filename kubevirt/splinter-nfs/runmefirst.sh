...
VERSION='24.04.1'
if [ ! -f "ubuntu-${VERSION}-server-amd64.iso" ]; then
	wget "https://old-releases.ubuntu.com/releases/24.04/ubuntu-${VERSION}-live-server-amd64.iso" -O "ubuntu-${VERSION}-server-amd64.iso"
fi
find_old() {
    while read -r -d '' file; do                  # read the output of find one file at a time
      files+=("$file")                            # append to the array
    done < <(find "ubuntu-${VERSION}-server-amd64.iso" -mtime +7 -print0) # generate NUL separated list of files
    if ((${#files[@]} == 0)); then
      # no files found
      return 10
    else
...
if [[ $(find_old | echo $?) -gt 0 ]]; then
	rm "ubuntu-${VERSION}-server-amd64.iso"
	wget "https://old-releases.ubuntu.com/releases/24.04/ubuntu-${VERSION}-live-server-amd64.iso" -O "ubuntu-${VERSION}-server-amd64.iso"
fi
