...
in
{
  ...
  # Create /run/runc directory for nvidia-container-runtime
  systemd.tmpfiles.rules = lib.mkIf hasNvidia [
    "d /run/runc 0755 root root -"
  ];
    ...
    (lib.mkIf hasNvidia {
      ...
      nvidia-container-cli-setup = {
        ...
        serviceConfig = {
          Type = "oneshot";
          RemainAfterExit = true;
          ExecStart = pkgs.writeShellScript "nvidia-setup" ''
            mkdir -p /usr/bin /etc/nvidia-container-runtime /run/nvidia/driver-bin
            ln -sf ${pkgs.libnvidia-container}/bin/nvidia-container-cli /usr/bin/nvidia-container-cli
            ln -sf ${pkgs.nvidia-container-toolkit.tools}/bin/nvidia-cdi-hook /usr/bin/nvidia-cdi-hook
            ln -sf ${pkgs.nvidia-container-toolkit}/bin/nvidia-ctk /usr/bin/nvidia-ctk
            cp -f ${pkgs.nvidia-container-toolkit.tools}/bin/nvidia-container-runtime-hook /usr/bin/nvidia-container-runtime-hook
            chmod +x /usr/bin/nvidia-container-runtime-hook
            ln -sfn /run/opengl-driver /run/nvidia/driver
            ln -sf ${config.hardware.nvidia.package}/bin/nvidia-smi /run/nvidia/driver-bin/nvidia-smi
            rm -f /etc/nvidia-container-runtime/config.toml
            ${pkgs.nvidia-container-toolkit}/bin/nvidia-ctk config --config-file=/etc/nvidia-container-runtime/config.toml --in-place
            ${pkgs.nvidia-container-toolkit}/bin/nvidia-ctk config --config-file=/etc/nvidia-container-runtime/config.toml --set nvidia-container-cli.path=/run/nvidia/driver-bin --in-place
            ${pkgs.nvidia-container-toolkit}/bin/nvidia-ctk config --config-file=/etc/nvidia-container-runtime/config.toml --set nvidia-container-runtime.mode=legacy --in-place
            ${pkgs.nvidia-container-toolkit}/bin/nvidia-ctk config --config-file=/etc/nvidia-container-runtime/config.toml --set nvidia-container-runtime-hook.path=/usr/bin/nvidia-container-runtime-hook --in-place
            ${pkgs.nvidia-container-toolkit}/bin/nvidia-ctk config --config-file=/etc/nvidia-container-runtime/config.toml --set nvidia-container-cli.no-cgroups=true --in-place
            sed -i 's|#debug = "/var/log/nvidia-container-runtime.log"|debug = "/var/log/nvidia-container-runtime.log"|' /etc/nvidia-container-runtime/config.toml
          '';
        };
      };
