...
echo "=== SteamVR Container Starting ==="

# Add users (both legacy and auth systems)
echo "Creating users..."
bash /usr/bin/create-users.sh

# Set up games directory with proper permissions
if [ -d /workspace/games ]; then
    echo "Setting up games directory permissions..."
    # Find the first user from createusers.txt if it exists
    if [ -f /root/createusers.txt ]; then
        MAIN_USER=$(head -n1 /root/createusers.txt | cut -d: -f1)
        if [ -n "$MAIN_USER" ] && id "$MAIN_USER" &>/dev/null; then
            chown -R "$MAIN_USER:$MAIN_USER" /workspace/games
            chmod -R 755 /workspace/games
            echo "Games directory owned by $MAIN_USER"
        fi
    fi
fi

# Install NVIDIA driver matching host version (disabled for NixOS - uses host driver)
# /root/scripts/install-nvidia-driver.sh
