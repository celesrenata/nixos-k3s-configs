...
COMMANDS[6]='source ~/miniforge3/etc/profile.d/conda.sh && conda activate llm-cpp && yes | pip install --pre --upgrade ipex-llm[xpu] --extra-index-url https://pytorch-extension.intel.com/release-whl/stable/xpu/us/
...
source ~/miniforge3/etc/profile.d/conda.sh && conda activate llm-cpp && huggingface-cli login --token "YOUR_HF_TOKEN_HERE"
cd llama-cpp && wget https://huggingface.co/lmstudio-community/Meta-Llama-3-8B-Instruct-GGUF/resolve/main/Meta-Llama-3-8B-Instruct-Q8_0.gguf'

COMMANDS[7]='source ~/miniforge3/etc/profile.d/conda.sh && conda activate llm-cpp && source /opt/intel/oneapi/setvars.sh && cd llama-cpp && ZES_ENABLE_SYSMAN=1 ./server -m Meta-Llama-3-8B-Instruct-Q8_0.gguf -n 32 -t 8 -ngl 33 --no-mmap > /tmp/llama.cpp'
